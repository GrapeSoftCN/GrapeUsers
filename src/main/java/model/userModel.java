...
public class userModel
{
  ...
  public String checkLogin(JSONObject userinfo) {
    ...
    try {
      ...
      case 0:
        ...
        if (!userinfo.containsKey("password")) break;
      ...
      if (rs != null) {
        ...
        if (rs != null) {
          ...
          if ((array != null) && (array.size() > 0)) {
            ...
            int i = 0; for (int len = array.size(); i < len; i++) {
  ...
  private String getRole(String ugid) {
    ...
    String temp = appsProxy.proxyCall("/GrapeUser/roles/getRole/" + ugid).toString();
  ...
  public void logout(String sid) {
    ...
    if ((GrapeSID == null) && 
      (!this.ssessionmap.containsKey(sid))) {
    ...
    if (userInfo != null) {
      try {
        if ((userInfo.containsKey("id")) && 
          (!checkUserName(userInfo.get("id").toString()))) {
          return 2;
        }

        if ((userInfo.containsKey("email")) && 
          (!checkEmail(userInfo.get("email").toString()))) {
          return 4;
        }

        if ((userInfo.containsKey("mobphone")) && 
          (!checkMobileNumber(userInfo.get("mobphone").toString()))) {
          return 6;
        }
  ...
  public String select(JSONObject userInfo) {
    ...
    if (userInfo != null) {
      try {
        array = new JSONArray();
        for (Iterator localIterator = userInfo.keySet().iterator(); localIterator.hasNext(); ) { Object object2 = localIterator.next();
  ...
  public String page(String wbid, int idx, int pageSize, String userInfo)
  {
    JSONObject object = new JSONObject();
    JSONArray array = null;
    long total = 0L; long totalSize = 0L;
    if (isAdmin())
      wbid = null;
    else {
      wbid = currentWeb;
